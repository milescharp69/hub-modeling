...
def app():
    ...
    metric_container_info_col1.metric("Usage 7am-10pm", Hub_Commercial_Dominant.Notional_Loading[0], pagedata.usageAdelta)
    metric_container_info_col2.metric("Usage 10pm-7am", Hub_Commercial_Dominant.Notional_Loading[1], pagedata.usageBdelta)
    ...
    maincol1expander.markdown("<p style='text-align: left; color: black; text-indent: 15%;'>Hub Type:       {}</p>".format(Hub_Commercial_Dominant.Hub_Type), unsafe_allow_html=True)
    ...
    for i in range(len(Hub_Commercial_Dominant.ESVE_Ports)):
        maincol1expander.markdown(
            "<p style='text-align: left; color: black; text-underline-offset: 20%; text-indent: 20%;'>• {}</p>".format(Hub_Commercial_Dominant.ESVE_Ports[i].datatext()),
            unsafe_allow_html=True)
